...
// Size in bytes of sine wave samples we'll create on each loop pass         
   ...
   SourceDataLine line;
   private double fFreq;                                    //Set from the pitch slider
   ...
   public void run() {
      ...
      while (bExitThread==false) {
         ...
         for (int i=0; i<SINE_PACKET_SIZE/SAMPLE_SIZE; i++) {
            cBuf.putShort((short)(Short.MAX_VALUE * Generator.sin(fCyclePosition, fFreq, 0)));
fCyclePosition++;
...
//               fCyclePosition -= 1;
   ...
   public synchronized void changeFreq(double freq)
   {
	   this.fFreq=freq;
   }
